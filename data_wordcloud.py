...
def china_wordcloud():
    ...
    word_cloud = wordcloud.WordCloud(font_path='static/simsun.ttc',
                                     background_color='#CDC9C9',
                                     min_font_size=15,
                                     width=900, height=500)
    word_cloud.generate_from_frequencies(china_dict)
    word_cloud.to_file('static/assets/img/WordCloud-China.png')
    print('中国省份疫情词云图绘制完毕！')
...
def global_wordcloud():
    ...
    word_cloud.to_file('static/assets/img/WordCloud-Global.png')
    print('全球各国疫情词云图绘制完毕！')
...
def all_wordcount():
    ...
    page = Page(layout=Page.SimplePageLayout, page_title="疫情词云展示")
    page.add(
        china_wordcloud(),
        global_wordcloud()
    )
